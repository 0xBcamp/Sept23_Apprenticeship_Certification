...
contract OrganizationResolver is SchemaResolver {
    ...
    constructor(IEAS eas, string memory _organizationName, address[] memory initialMembers) SchemaResolver(eas) {
        require(initialMembers.length > 0, "Must provide at least one member address");
    ...
    function onAttest(EASAttestation calldata easAttestation, uint256 /*value*/) internal override returns (bool) {
        require(isMentor[easAttestation.attester] || isOrganizer[easAttestation.attester], "Not listed as mentor or organizer, please whitelist the address");
    ...
    function onRevoke(EASAttestation calldata easAttestation, uint256 /*value*/) internal view override returns (bool) {
        require(block.timestamp <= attestationTimestamps[easAttestation.attester] + REVOKE_WINDOW, "Revocation time window has passed");
    ...
    function isOrganizationMember(address _address) external view returns (bool) {
