...
contract CertificateAttestation is SchemaResolver, ERC721Enumerable {
    ...
    constructor(IEAS eas, string memory name, string memory symbol) SchemaResolver(eas) ERC721(name, symbol) {
        owner = msg.sender;
    }

    function addToMentors(address _mentor, string memory templateIPFS) external onlyOwner {
    ...
    function addToOrganizers(address _organizer, string memory templateIPFS) external onlyOwner {
    ...
    function mintCertificate(address candidate, string memory metadataURI, string memory stampedIPFSHash) external {
        require(isMentor[msg.sender] || isOrganizer[msg.sender], "Not listed as mentor or organizer");
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_isTokenOwnerOrApproved(tokenId, msg.sender), "ERC721Metadata: URI query for nonexistent token");
        return certificateData[tokenId].ipfsHash;
    }

    function _isTokenOwnerOrApproved(uint256 tokenId, address account) internal view returns (bool) {
        address tokenOwner = ownerOf(tokenId);
        return (tokenOwner == account || getApproved(tokenId) == account || isApprovedForAll(tokenOwner, account));
    }

    function getCertificateMetadataURI(uint256 tokenId) external view returns (string memory) {
