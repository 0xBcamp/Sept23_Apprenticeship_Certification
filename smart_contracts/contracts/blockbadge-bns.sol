...
contract BlockBadgeBNS is Ownable {
    ...
    modifier nameAvailable(string memory bnsName) {
        require(nameToAddress[bnsName] == address(0), "BNS name is already taken");
        _;
    }

    // Modifier to ensure the sender owns the BNS name
    modifier ownsName(string memory bnsName) {
        require(nameToAddress[bnsName] == msg.sender, "You do not own this BNS name");
        _;
    }
     ...
     */
    function registerName(string memory bnsName) external nameAvailable(bnsName) {
        require(bytes(bnsName).length > 0, "BNS name cannot be empty");

        string memory fullBNS = string(abi.encodePacked(bnsName, ".blockbadge"));
     ...
     */
    function resolveName(string memory bnsName) external view returns (address) {
     ...
     */
    function transferName(string memory bnsName, address to) external ownsName(bnsName) {
